...
(function($){
    ...
    Wikipedia = {
        ...
        Get: function(wikipediaPage, options, callback){
            ...
            $.getJSON('https://'+Wiki.settings.locale+'.wikipedia.org/w/api.php?action=parse&format=json&callback=?', {page:wikipediaPage, prop:'text|images', uselang: Wiki.settings.locale}, function(parsedata) {
                ...
                if (typeof parsedata.error !== "undefined") {
                    ...
                    if (parsedata.parse.text["*"].substring(0, 3) == '<p>' || parsedata.parse.text["*"].substring(0, 25) == '<div class="redirectMsg">') {
                        ...
                        if (typeof $(data).find('.infobox .image img').attr('src') !== "undefined") {
                            Wiki.article.logo = 'https:' + $(data).find('.infobox .image img').attr('src');
                        } else {
                        ...
                        $.each(data.find('.thumb a.image img'), function (index, image) {
                            Wiki.article.gallery.push('https:' + $(image).attr('src'))
                        });
