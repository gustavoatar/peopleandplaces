...
if ('webkitSpeechRecognition' in window) {
  ...
  recognition.onstart = function() {
	...
	jQuery('.next-arrow, .prev-arrow, header, .video-control, .video-control-play').addClass('hide');
  };
  ...
  recognition.onend = function() {	
	...
	jQuery('.next-arrow, .prev-arrow, header, .video-control, .video-control-play').removeClass('hide');
...
jQuery(document).ready(function($){
	...
	{
		strings: ['From home, from anywhere, just go.'],
		loop: false,
		typeSpeed: 35,
		backSpeed: 10,
		backDelay: 1200,
...
// 		}							
		...
		if (voiceInput.search('/([A-Za-z]+(?: [A-Za-z]+)*),? ([A-Za-z]{2})/')){
			...
			jQuery('.year-entry').val(cityName);
			jQuery('.typed-out').addClass('fadeOut').addClass('hide');
			jQuery('.retry-text').removeClass('hide').addClass('fadeIn'); 
			jQuery('.searchInstagram').click();			
		}
